...
RSpec.describe GeosparqlToGeojson::Converter::PolygonConverter do
  ...
  context '#find_min_and_max_axis' do
    before do
      subject.send(:find_min_and_max_axis, eval(IO.read('spec/fixtures/method_outputs/polygons')).first)
    end
  ...
  context '#add_formatted_polygons_to_data_hash' do
    before do
      subject.instance_variable_set(:@data_store, {})
      subject.instance_variable_set(:@formatted_polygon_array, [1, 2, 3])
      subject.send(:add_formatted_polygons_to_data_hash)
    end

    it 'will populate @data_store with formatted polygons' do
      expect(subject.instance_variable_get(:@data_store)).to eq(Polygon: [1, 2, 3])
    end
  end
end
